...
from utils.PathManager import store
import math
import time
...
def init_datasets():
    get_data(VDataSet.ImageNet, data_type="train")


def init_dataloader():
    loader = get_data_loader(VDataSet.CIFAR10, data_type="train",
...
def test_hrank():
    ...
    test_loader = get_data_loader(VDataSet.CIFAR10, data_type="test", batch_size=32,
                                  shuffle=True, num_workers=0, pin_memory=False)

    hrank_obj = VGG16HRank(modelUtil.vgg_16_bn(ORIGIN_CP_RATE))
    # hrank_obj.wrapper.load_checkpoint(file_repo.wrapper(True))
    ...
    # wrapper = hrank_obj.cp_model
    # from thop import profile
    # flops, params = profile(wrapper, inputs=(inputs,))
    # print(flops, "=====", params)
...
def resnet56_cifar100_single_convergence():
    loader = get_data_loader(VDataSet.CIFAR100, data_type="train",
                             batch_size=32, shuffle=True,
                             num_workers=0, pin_memory=True)
    global_logger.info("Sampler initialized")
    ...
    test_loader = get_data_loader(VDataSet.CIFAR100, data_type="test", batch_size=32,
                                  shuffle=True, num_workers=0, pin_memory=True)
...
def union_convergence():
    ...
    workers_loaders = get_data_loader(VDataSet.CIFAR10, data_type="train",
